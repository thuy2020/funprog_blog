...
plots_workplaces <- d %>% # assign to an object
  group_by(country) %>% 
  nest() %>% 
  mutate(plots = pmap(list(country, data), # call columns "country" and "data" as inputs, create new column for plots  
                      ~ggplot(..2, aes(date, workplaces)) + # ..2 to select 2nd column for plot data
                        geom_point() + 
                        geom_line()))
...
Some final adjustments to the plots
```{r data viz improvements}
...
# This is one way. But how to do it by looping through lm_by_country_lists rather than facet wrapping? 
